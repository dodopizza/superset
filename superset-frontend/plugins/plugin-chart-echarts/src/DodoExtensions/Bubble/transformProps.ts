...
const getTooltipLabel = (
  ...
  if (typeof value === 'string') {
    ...
    if (metric) {
      const key = `verbose_name_${locale}` as keyof Metric;
...
export default function transformProps(chartProps: BubbleDodoTransformProps) {
  ...
  const tooltipLabels: BubbleDodoComponentProps['tooltipLabels'] = {
    x: getTooltipLabel(axisXInfo, metrics, 'x', locale),
    y: getTooltipLabel(axisYInfo, metrics, 'y', locale),
    size: getTooltipLabel(bubbleSizeInfo, metrics, 'size', locale),
  };
