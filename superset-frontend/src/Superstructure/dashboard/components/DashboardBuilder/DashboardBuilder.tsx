...
// Needed for top level tabs rendering the native filters on load
// let initPage = false;
...
const TopLevelTabsWrapper = styled.div`
  margin-top: 0;
`;
...
const DashboardBuilder: FC<DashboardBuilderProps> = () => {
  ...
  const editMode = useSelector<RootState, boolean>(
    state => state.dashboardState ? state.dashboardState.editMode : false,
  );
  ...
  return (
    <StyledDiv id="one_id">
      ...
      <StyledHeader>
        <TopLevelTabsWrapper
          style={{ marginLeft: !dashboardFiltersOpen ? '0px' : '32px' }}
        >
          <DashboardComponent
            id={topLevelTabs?.id}
