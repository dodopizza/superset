...
const getProxyConfig = require('./standalone.webpack.proxy-config');
...
const plugins = [
  ...
  new HtmlWebpackPlugin({
    template: `${APP_DIR}/src/assets/staticPages/404.html`,
    inject: true,
    chunks: [],
    filename: '404.html',
  }),
  new HtmlWebpackPlugin({
    template: `${APP_DIR}/src/assets/staticPages/500.html`,
    inject: true,
    chunks: [],
    filename: '500.html',
  }),
...
const config = {
  ...
  module: {
    ...
    rules: [
      ...
      {
        test: /\.tsx?$/,
        exclude: [/Superstructure.*$/, /\.test.tsx?$/],
        use: [
          'thread-loader',
          babelLoader,
          {
...
if (isDevMode) {
  ...
  config.devServer = {
    ...
    contentBase: path.join(process.cwd(), `${APP_DIR}/static/assets`),
  };
